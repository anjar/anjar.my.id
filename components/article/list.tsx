    ...
    {posts.map(({ title, slug, image }: DocumentTypes, idx: number) => {
      ...
      return (
        <article
          ...
          className="relative w-full h-64 bg-cover bg-center group overflow-hidden shadow-lg hover:shadow-2xl  duration-300 rounded-lg transition ease-in-out transform hover:-translate-y-1"
          style={bgStyle}
        >
          {image && (
            <Image
