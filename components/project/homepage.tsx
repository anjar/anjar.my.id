...
const LatestProject: FunctionComponent = () => {
  ...
  return (
    <ul className="flex flex-col md:flex-row flex-wrap gap-3">
      {project.map(
        ({ name, description, url, homepageUrl }: ProjectType, idx: number) => {
          const colsLink = homepageUrl ? "grid-cols-2" : "grid-cols-1";
          return (
            <li
              key={`posts_${idx.toString()}`}
              className="border-gray-400 flex flex-col md:flex-row mb-2"
            >
                  ...
                  {homepageUrl && (
                    ...
                    >
                      <FontAwesomeIcon
                        icon={faLink}
                        fontSize="1.5rem"
                        className="hover:text-gray-300 dark:hover:text-gray-900"
                      />
                    </a>
                  )}
